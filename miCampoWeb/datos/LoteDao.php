...
include '../entidades/Lote.php';

class LoteDao extends Conexion{
    ...
    //Metodo sirve para obtener los campos de un Usuario
         public static function getLotePorCampoId($campo_id){

        $query =  "SELECT lote.campo_id , lote.lote_id , lote.nombre , lote.tamano, sum(lat1+lat2+lat3+lat4)/4 as lat1, sum(long1+long2+long3+long4)/4  as long1, estado_lote.nombre as estado from lote 
            inner join estado_lote on (lote.estado_id = estado_lote.estado_id) WHERE lote.campo_id =:campo_id  group by lote.lote_id" ;
    ...
    public static function registrar($lote){

        $query = "INSERT INTO lote (nombre,apellido,correo,telefono,contrasena,privilegio_id) 
